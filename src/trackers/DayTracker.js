...
const createYearDocument = async (yearKey) => {
  try {
    const yearDocRef = doc(db, "rateMyDay", yearKey);
    const yearData = {};
...
const DayTracker = () => {
  ...
  const renderTable = () => {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      rows.push(
        <tr key={month}>
          <td className="day-tracker__month-label">
  ...
  const ratingColors = [
    { rating: 100, color: "PURPLE", hex: "#6b02f5" },
  ...
  return (
    <div className="day-tracker__container">
      <h1 className="day-tracker__header">Day Ratings - {yearKey}</h1>
      <div className="day-tracker__year-navigation">
        <button onClick={() => handleYearChange(-1)}>←</button>
        <button onClick={() => handleYearChange(1)}>→</button>
      </div>

        <div className="day-tracker__color-palette">
          {ratingColors.map((item) => (
            <button
              key={item.rating}
              className="day-tracker__color"
      ...
      <div className="day-tracker__content">
        ...
        <table className="day-tracker__calendar">
          <thead>
            <tr>
              <th>Month</th>
