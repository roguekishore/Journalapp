...
const createYearDocument = async (yearKey) => {
  try {
    const yearDocRef = doc(db, "moodTracker", yearKey);
    const yearData = {};
...
const MoodTracker = () => {
  ...
  const handleCellClick = async (month, day) => {
    ...
    try {
      ...
      console.log(`Updated mood for day ${day} of month ${month}`);
    } catch (error) {
  ...
  const renderTable = () => {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      rows.push(
        <tr key={month}>
          <td className="mood-tracker__month-label">
  ...
  return (
    <div className="mood-tracker__container">
      <h1 className="mood-tracker__header">Mood Tracker - {yearKey}</h1>
      <div className="mood-tracker__year-navigation">
        <button onClick={() => handleYearChange(-1)}>←</button>
        <button onClick={() => handleYearChange(1)}>→</button>
      </div>

      <div className="mood-tracker__color-palette">
        {moodColors.map((item) => (
          <button
            key={item.rating}
            className="mood-tracker__color"
      ...
      <div className="mood-tracker__content">
        ...
        <table className="mood-tracker__calendar">
          <thead>
            <tr>
              <th>Month</th>
