...
const createYearDocument = async (yearKey) => {
  try {
    const yearDocRef = doc(db, "sleepTracker", yearKey);
    const yearData = {};
...
const SleepTracker = () => {
  ...
  const renderTable = () => {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      rows.push(
        <tr key={month}>
          <td className="sleep-tracker__month-label">
  ...
  return (
    <div className="sleep-tracker__container">
      <h1 className="sleep-tracker__header">Sleep Quality - {yearKey}</h1>
      <div className="sleep-tracker__year-navigation">
        <button onClick={() => handleYearChange(-1)}>←</button>
        <button onClick={() => handleYearChange(1)}>→</button>
      </div>

        <div className="sleep-tracker__color-palette">
          {ratingColors.map((item) => (
            <button
              key={item.rating}
              className="sleep-tracker__color"
      ...
      <div className="sleep-tracker__content">

        <table className="sleep-tracker__calendar">
          <thead>
            <tr>
              <th>Month</th>
